...
export default class Transaction {
    ...
    isValid(): Validation {
        ...
        const inputsSum= this.txInputs.map(txi=>txi.amount).reduce((a,b)=>a+b,0);
        const outputsSum= this.txInputs.map(txo=>txo.amount).reduce((a,b)=>a+b,0);
        if (inputsSum>outputsSum){
            return new Validation(false,"Invalid tx: amounts must be equals or greater than outputs amounts")
        }
